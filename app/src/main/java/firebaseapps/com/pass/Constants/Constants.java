package firebaseapps.com.pass.Constants;

/**
 * Created by 1405214 on 23-09-2016.
 */

public class Constants {

    /**
     *  SMS Constants
     */
    public static String SMS_URL="http://103.233.79.246/submitsms.jsp";
    public static String SMS_PARAM_KEY_ACCUSAGE="accusage";
    public static String SMS_PARAM_VALUE_ACCUSAGE="1";

    public static String SMS_PARAM_KEY_SENDERID="senderid";
    public static String SMS_PARAM_VALUE_SENDERID="INFOSM";

    public static String SMS_PARAM_KEY_MESSAGE="message";

    public static String SMS_PARAM_KEY_MOBILE="mobile";

    public static String SMS_PARAM_KEY_USER="user";
    public static String SMS_PARAM_VALUE_USER="SACHIN";

    public static String SMS_PARAM_KEY_KEY="key";
    public static String SMS_PARAM_VALUE_KEY="96f84f2a28XX";

    /**
     * Email Params
     */
    public static String EMAIL_SENDER="dwijkishan@gmail.com";
    public static String EMAIL_PASSWORD_SENDER="dwijkishan214224";

    /**
     *
     */

    public static String PASS_UPLOAD_PARAM_NAME="Name";
    public static String PASS_UPLOAD_PARAM_ADDRESS="ame";
    public static String PASS_UPLOAD_PARAM_ID_NO="Nae";
    public static String PASS_UPLOAD_PARAM_PURPOSES="Nam";
    public static String PASS_UPLOAD_PARAM_DATE_OF_BIRTH="sName";
    public static String PASS_UPLOAD_PARAM_DATE_OF_JOURNEY="dName";

    public static String REGISTRATION_USER_MOBILE_KEY="user_mobile";
    public static String REGISTRATION_USER_EMAIL_KEY="user_email";


    /**
     * Offline Links
     */
    public static String OFFLINE_REGISTRATION_URL="http://192.168.2.2/pass3/index.php/api/userapi/users/format/json";
    public static String IMAGE_SEND_LINK= "http://192.168.2.2/pass3/index.php/api/userapi/userpassdetail/format/json";
    public static String PRICING_URL="http://192.168.2.2/pass3/index.php/api/userapi/place/format/json";
    public static String CONFIRMATION_LINK="http://192.168.2.2/pass3/index.php/api/userapi/updateprice_status/format/json";
    public static String OFFLINE="http://192.168.2.2/pass3/index.php/api/userapi/check/format/json";
    public static String UNAVAILABLE_DOJ="http://192.168.2.2/pass3/index.php/api/userapi/datesl_rejects_list/format/json";
    public static String PASS_RETREIVE_URL="http://192.168.2.2/pass3/index.php/api/userapi/user_pass_info/format/json";
    public static String GET_APPLICANT_STATUS="";
    public static String CANCEL_REQUEST="";
    public static String VEHICLE_DETAILS_CHANGE_LINK="http://192.168.2.2/pass3/index.php/api/userapi/updates_user_vehicle_detail/format/json";
    public static String PROFILE_CHANGE_LINK="http://192.168.2.2/pass3/index.php/api/userapi/update_user_pass_detail/format/json";
    public static String VEHICLE_DETAILS_UPDATE_LINK="http://192.168.2.2/pass3/index.php/api/userapi/uservehicledetail/format/json";
    public static String UPDATE_DETAILS_URL="http://192.168.2.2/pass3/index.php/api/userapi/price_updation_pass/format/json";
    public static String STATUS_DETAIL_URL="";
    public static String GET_APPLICANT_MOB_CHANGE_DETAIL="http://192.168.2.2/pass3/index.php/api/userapi/re_schedule_phone/format/json";
    public static String SHARED_PREFS_NAME="USER";
    public static String SHARED_PREF_KEY="USER_MOBILE";
    public static  String GET_REUPLOAD_DETAILS_LINK="http://192.168.2.2/pass3/index.php/api/userapi/user_applic_detail_status/format/json";

    /**
     * ONLINE LINKS
     */
    public static String ONLINE_REGISTRATION_URL="http://innovadorslab.co.in/pass3/index.php/api/userapi/users/format/json";
    public static String ONLINE_IMAGE_SEND_LINK= "http://innovadorslab.co.in/pass3/index.php/api/userapi/userpassdetail/format/json";
    public static String ONLINE_PRICING_URL="http://innovadorslab.co.in/pass3/index.php/api/userapi/place/format/json";
    public static String ONLINE_CONFIRMATION_LINK="http://innovadorslab.co.in/pass3/index.php/api/userapi/updateprice_status/format/json";
    public static String ONLINE_UNAVAILABLE_DOJ="http://innovadorslab.co.in/pass3/index.php/api/userapi/datesl_rejects_list/format/json";
    public static String ONLINE_PASS_RETREIVE_URL="http://innovadorslab.co.in/pass3/index.php/api/userapi/user_pass_info/format/json";
    public static String ONLINE_VEHICLE_DETAILS_CHANGE_LINK="http://innovadorslab.co.in/pass3/index.php/api/userapi/updates_user_vehicle_detail/format/json";
    public static String ONLINE_PROFILE_CHANGE_LINK="http://innovadorslab.co.in/pass3/index.php/api/userapi/update_user_pass_detail/format/json";
    public static String ONLINE_VEHICLE_DETAILS_UPDATE_LINK="http://innovadorslab.co.in/pass3/index.php/api/userapi/uservehicledetail/format/json";
    public static String ONLINE_UPDATE_DETAILS_URL="http://innovadorslab.co.in/pass3/index.php/api/userapi/price_updation_pass/format/json";
    public static String ONLINE_GET_APPLICANT_MOB_CHANGE_DETAIL="http://innovadorslab.co.in/pass3/index.php/api/userapi/re_schedule_phone/format/json";
    public static String ONLINE_GET_REUPLOAD_DETAILS_LINK="http://innovadorslab.co.in/pass3/index.php/api/userapi/user_applic_detail_status/format/json";
    public static String ONLINE_CANCEL_REQUEST="";
    public static String ONLINE_GET_PASS_URL="http://innovadorslab.co.in/pass3/index.php/api/userapi/view_pass_info/format/json";

}
